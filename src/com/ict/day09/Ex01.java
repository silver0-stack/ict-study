...
public class Ex01 {
	public static void main(String[] args) {
		 ...
		 *			- static 메모리에 생성된다.
		 *		
		 *		** 메소드 안에 있는 지역변수는 해당 메소드가 실행될 때 만들어진다.
		 *			해당 메소드가 끝나면 지역변수는 소멸된다.
		 *
		 *      
		 *      [메모리]
		 *      - static: 프로그램이 시작 전부터 끝까지 존재하는 메모리 영역
		 *      		  static 필드, static 메소드
		 *      - heap: 프로그램이 시작할 때 있거나 중간에 생성되다가 어느 순간에 없어진다(동적할당)
		 *      		EX) 클래스 객체 etc
		 *      - stack: heap 메모리에 생성된 obj 타입 데이터의 참조값(주소)을 할당한다.
		 *      		primitive 타입의 데이터가 값과 함께 할당된다. (int, char, double...)
		 *      		원시타입(기본자료형)의 데이터들이 할당된다.
		 *      		기본자료형은 실제 데이터 값이 stack에 저장된다
		 *      
		 *     */
	}
}
